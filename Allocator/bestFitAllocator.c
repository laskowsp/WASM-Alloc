...
char doMetrics = 0; // If this is 1, we will store the maximum heap size and average free space remaining at heap expansion
...
int malloc(int size){
    ...
    int position = firstAllocated;
    // Variables to perform best fit allocation with
    int chosenPosition = -1;
    int chosenSizeDiff = 0;

    // If we haven't allocated anything yet, then we allocate to 0
    if (firstAllocated == -1){
        firstAllocated = 0;
        return allocate(0, size, 0);
    }

    // If we have something allocated but not at the start of the memory region, then set our chosen values for it if we can fit it in
    int firstSpace = firstAllocated - size - (int)sizeof(AllocBlock);
    if (firstSpace >= 0){
        chosenPosition = 0;
        chosenSizeDiff = firstSpace;
    }

    while (1){
        if(getNext(position) == 0){
            // Nothing after this one - if we've found a best fit, alloc there, otherwise alloc to the end
            if (chosenPosition == -1) {
                // Nope, to the end it goes
                ...
                // Special handling if this is the first block - that means we don't need to worry about moving anything around
                if (chosenPosition == 0 && firstAllocated != 0){
                    unsigned short ret = allocate(0, size, firstAllocated);
                    firstAllocated = 0;
                    return ret;
                } else {
                    // We have a best block to allocate to - allocate there
                    unsigned short nextPos = getNext(chosenPosition);
                    unsigned short ret = allocate(chosenPosition + getSize(chosenPosition), size, nextPos);
                    setNext(chosenPosition, ret);
                    if (chosenPosition == 0){
                        firstAllocated = 0;
                    }
                    return ret;
            ...
            // We're not at the end yet, see if this is position is better
            int remainingSize = getNext(position) - size - (position + getSize(position)) - (int)sizeof(AllocBlock);
            // If we have room here, and we either don't have a selected chosen one or it's a better fit, set those values accordingly
            if (remainingSize >= 0 && (remainingSize < chosenSizeDiff || chosenPosition == -1)){
                chosenPosition = position;
                chosenSizeDiff = remainingSize;
            }
        }
